-- get_view (the card/listing source) was redefined by the keyword-search migration
-- without banner_url (a regression) and never carried headline. Cards want both
-- (item.headline for the short description, item.bannerURL for the thumbnail), so
-- add them back. Only the SELECT list and GROUP BY change; the parameterized filter
-- args are untouched.

-- Drop a stale lower-arity overload left behind by an earlier signature change, so the
-- function name resolves unambiguously (otherwise PostgREST raises PGRST203).
        ...
        'SELECT JSON_AGG(t)
         FROM (
             ...
             WHERE p.latest_mautic_support = TRUE
               ...
               ' || date_filter || '
             GROUP BY p.name, p.url, p.repository, p.description, p.headline, p.banner_url, p.downloads, p.favers, p.type, p.displayname, p.language, p.time, p.created_at
