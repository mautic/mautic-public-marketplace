...
final class MarketplaceControllerTest extends WebTestCase
{
    ...
    public function testPaidPackageDownloadRedirectsAnonymousUsersToLogin(): void
    {
        $client = self::createClient();
        $client->request('GET', '/package/mautic/zebra-theme/download');

        self::assertResponseRedirects('/auth/login?returnTo=/package/mautic/zebra-theme/download');
    }

    public function testPaidPackageDownloadRedirectsUnpaidUsersToCheckout(): void
    {
        $client = self::createClient();
        $client->loginUser(new Auth0User('auth0|test123', 'Test User', 'test@example.com', null), 'main');
        $client->request('GET', '/package/mautic/zebra-theme/download');

        self::assertResponseRedirects('/package/mautic/zebra-theme/buy');

        // The archive must not be served, so nothing may be recorded either.
        self::assertSame([], self::getContainer()->get(SupabaseMockHttpClient::class)->recordedDownloads);
    }
